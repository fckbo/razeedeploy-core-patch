...
module.exports = class BaseDownloadController extends CompositeController {
  ...
  async added() {
    console.log("fb!!!->BaseDownloadController");
    ...
    for (var i = 0; i < requests.length; i++) {
      ...
      console.log("fb!!!-BaseDownloadController->reqOpt=",reqOpt);
      ...
      try {
        console.log("fb!!!-BaseDownloadController->download->reqOpt=",reqOpt);
        let res = await this.download(reqOpt);
        if (res.statusCode >= 200 && res.statusCode < 300) {
          this.log.debug(`Download ${res.statusCode} ${url}`);
          console.log("fb!!!->BaseDownloadController->safeLoadAll", res.body);
          file = yaml.safeLoadAll(res.body);

          console.log("fb!!!->BaseDownloadController<-safeLoadAll", file);
  ...
  async _decomposeFile(file) {
    ...
    console.log("fb!!!-_decomposeFile=",file);
